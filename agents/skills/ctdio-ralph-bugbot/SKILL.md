...
description: Skill for addressing bugbot feedback and CI failures on PRs. Fetches unresolved bugbot comments, triages them, fixes valid bugs, resolves false positives, checks CI status, fixes lint/format/test failures, and commits changes.
color: orange
---

# Ralph Bugbot

Skill for systematically addressing bugbot feedback and CI failures on pull requests.
...
### Step 1: Orient and Wait

Before doing anything else:

1. **Check what's already been done** — run `git log --oneline -10` and `git diff HEAD~3..HEAD --stat` to see if prior work addressed any bugbot comments or CI failures
2. **Wait for bugbot to settle** — run `wait-for-bugbot.sh`. A prior session may have pushed changes that bugbot is still analyzing. Do not proceed until bugbot reaches a terminal state.

This ensures you start with a current picture of what bugbot actually thinks about the latest code, not stale state from before a push.
...
If the PR is clean, you are done. There is nothing left to do.

**If you could NOT achieve a clean state** (unfixable CI failure, pending timeout after retries, etc.), describe what remains factually:
...
**You MUST wait for all checks to reach a terminal state before concluding.** Specifically:

- If `wait-for-bugbot.sh` reports bugbot is still running or pending → **wait for it to complete, then re-fetch comments**
- If `check-ci-status.sh` exits with code 2 (timeout/pending) → **sleep 60 seconds and re-run the script. Repeat up to 3 successive times.** After 3 retries, report the pending checks but you are NOT done.
- **Step 6 is MANDATORY after every push** — every `gt submit` invalidates state. You must re-verify bugbot AND CI from scratch.
- **Pending is not passing** — a pending check has not reached a terminal state
- **All checks must settle** — do not describe any check as "external" or "not blocking"
- **CI verification is not optional** — CI status must always be checked and must always reach a terminal state before you can conclude
...
9. **Grind until clean** - After every push, re-verify bugbot AND CI from scratch (Step 6). Keep going until a full verification pass completes with nothing to fix.
...
14. **You are not done until everything has settled** - If any check is pending, any script is polling, or bugbot hasn't finished analyzing, you are not done. Do not rationalize pending checks as "status indicators" or "external checks." Wait until everything reaches a terminal state.
15. **Pending is not passing** - If anything is still pending, you cannot conclude. Go back and wait for every pending item to reach a terminal state before reporting.
16. **Partial success is not resolution** - If bugbot comments are at zero but CI is failing, you are NOT done. If CI is passing but bugbot comments remain, you are NOT done. All completion criteria (Step 6d) must be true simultaneously in a single verification pass with no pushes before you can say the PR is clean.
