...
Step 5: For each AVAILABLE capability, execute the harness scenarios
   → Create temp scripts in scratchpad directory
   → Run them, capture output
   → Record PASS/FAIL per scenario

Step 6: For each UNAVAILABLE capability, document reason as SKIPPED
   → Include what's missing (package, binary, server)
   → SKIPPED is non-blocking — don't fail because tooling is absent

Step 7: Compile TesterResult
